...
function searchCuisines(allDishes) {
    ...
    let userInput = customPrompt('Please enter cuisine choice',
    allDishes.map(dish => dish.cuisine.toLowerCase()));
    let foundSpecificTaste = allDishes.filter(function(taste){
        if(taste.cuisine.toLowerCase().includes(userInput)){
            return true;
        }
    })
...
function searchIngredients(allDishes) {
    ...
    let userInput = customPrompt('Please enter an ingredient to search for',
    allDishes.map(dish => dish.ingredients).flat());
    let foundSpecificIngredient = allDishes.filter(function(dish){
    ...
    return foundSpecificIngredient
}
...
function generateCuisineDishName(allDishes) {
    ...
    let fullMenuNames = allDishes.map(function(dish){
        return dish.name + '  ' + dish.cuisine
    })
    console.log(fullMenuNames);
...
function generateMarketingMessage(dishOfTheDay, messageTypeCallback) {
    ...
    let messageResult = messageTypeCallback(dishOfTheDay);
...
function customPrompt(promptQuestion, arrayOfValidResponses) {
    ...
    let response 
    do {
        response = prommpt(promptQuestion)
    } while (!arrayOfValidResponses.includes(response.toLowerCase()))
